...
func TestErasureCodeRecoverSimple(t *testing.T) {
	...
	fs := NewFFTSettings(5)
	poly := make([]ff.Fr, fs.MaxWidth, fs.MaxWidth)
	...
	debug.DebugFrs("poly", poly)
	// Get data for polynomial SLOW_INDICES
	data, err := fs.FFT(poly, false)
	if err != nil {
		t.Fatal(err)
	}
	debug.DebugFrs("data", data)

	// copy over the 2nd half, leave the first half as nils
	subset := make([]*ff.Fr, fs.MaxWidth, fs.MaxWidth)
	half := fs.MaxWidth / 2
	for i := half; i < fs.MaxWidth; i++ {
		subset[i] = &data[i]
	}

	debug.DebugFrPtrs("subset", subset)
	recovered, err := fs.ErasureCodeRecover(subset)
	if err != nil {
		t.Fatal(err)
	}
	debug.DebugFrs("recovered", recovered)
	for i := range recovered {
		if got := &recovered[i]; !ff.EqualFr(got, &data[i]) {
			t.Errorf("recovery at index %d got %s but expected %s", i, ff.FrStr(got), ff.FrStr(&data[i]))
		}
	}
	...
	debug.DebugFrs("back", back)
	for i := uint64(0); i < half; i++ {
		if got := &back[i]; !ff.EqualFr(got, &poly[i]) {
			t.Errorf("coeff at index %d got %s but expected %s", i, ff.FrStr(got), ff.FrStr(&poly[i]))
		}
	}
	for i := half; i < fs.MaxWidth; i++ {
		if got := &back[i]; !ff.EqualZero(got) {
...
func TestErasureCodeRecover(t *testing.T) {
	...
	randomSubset := func(known uint64, rngSeed uint64) []*ff.Fr {
		withMissingValues := make([]*ff.Fr, fs.MaxWidth, fs.MaxWidth)
		for i := range data {
